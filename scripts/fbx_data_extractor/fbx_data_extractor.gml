/// @description Functions to extract and process data from a parsed FBX node tree.
...
function _fbx_extract_single_mesh_data(geo_node, z_up) {
	...
	if (!variable_struct_exists(geo_node, "_children") ||
	    ...
	    !variable_struct_exists(geo_node._children, "LayerElementNormal") || !variable_struct_exists(geo_node._children.LayerElementNormal, "_children") || !variable_struct_exists(geo_node._children.LayerElementNormal._children, "Normals") ||
	    !variable_struct_exists(geo_node._children, "LayerElementUV") || !variable_struct_exists(geo_node._children.LayerElementUV, "_children") || !variable_struct_exists(geo_node._children.LayerElementUV._children, "UV"))
	{
		return -1;
	}
	...
	for (var i = 0; i < array_length(raw_indices);) {
		...
		for (var j = 2; j < array_length(poly_indices); j++) {
            ...
            // Reverted to the original order. The Y/Z coordinate swap above
            // already handles the winding order change.
